...
def main():
    # 创建输出目录
    output_dir = "test/proto2/generated"
    os.makedirs(output_dir, exist_ok=True)
    
    # 测试文件和期望生成的文件
    test_cases = [
        {
            'proto': 'test/proto2/test.proto',
            'expected_files': [
                'Character.gd',
                'Character_Inventory.gd',
                'Character_Item.gd',
                'GameSession.gd'
            ]
        },
        {
            'proto': 'test/proto2/complex.proto',
            'expected_files': [
                'ComplexMessage.gd',
                'ComplexMessage_NestedMessage.gd',
                'ComplexMessage_NestedMessage_DeepNested.gd',
                'TreeNode.gd',
                'NumberTypes.gd',
                'DefaultValues.gd',
                'FieldRules.gd'
